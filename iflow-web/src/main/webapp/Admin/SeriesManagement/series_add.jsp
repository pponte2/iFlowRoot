...
<% if (!done) { %>
  <input class="regular_button_01 btn btn-default" type="button" name="add" value="<%=messages.getString("button.add")%>" onClick="document.getElementById('series_add').<%=PARAM_ACTION%>.value='<%=ACTION_ADD%>';tabber_right(4, '<%=response.encodeURL("Admin/SeriesManagement/series_add.jsp")%>', get_params(document.series_add));" />
<% } %>
</fieldset>
</form>
